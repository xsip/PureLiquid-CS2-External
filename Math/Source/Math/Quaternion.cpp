...
constexpr Quaternion_t::Quaternion_t(const float x, const float y, const float z, const float w) :
	x(x), y(y), z(z), w(w) {
}

[[nodiscard]] bool Quaternion_t::IsValid() const
{
...
[[nodiscard]] Matrix3x4_t Quaternion_t::ToMatrix(const Vector& vecOrigin) const
{
...
QuaternionAligned_t& QuaternionAligned_t::operator=(const Quaternion_t& quatOther)
{
	this->x = quatOther.x;
	this->y = quatOther.y;
	this->z = quatOther.z;

// generated - do not edit!
// generated by @xsip cs2 schema generator

#pragma once

#include <cstddef>

namespace CS2 {
	namespace SchemaOffsets {
		namespace particles {
			// Class size: 0x208
			namespace C_OP_RemapDensityToVector /*: public CS2::particles::CParticleFunctionOperator*/ {
				constexpr std::ptrdiff_t m_flRadiusScale = 0x1d8; // float32 m_flRadiusScale; |  0x1d8 | Schema_Builtin | Size: 0x4
				constexpr std::ptrdiff_t m_nFieldOutput = 0x1dc; // particles::ParticleAttributeIndex_t  | Schema_DeclaredClass | Size: 0x4
				constexpr std::ptrdiff_t m_flDensityMin = 0x1e0; // float32 m_flDensityMin; |  0x1e0 | Schema_Builtin | Size: 0x4
				constexpr std::ptrdiff_t m_flDensityMax = 0x1e4; // float32 m_flDensityMax; |  0x1e4 | Schema_Builtin | Size: 0x4
				constexpr std::ptrdiff_t m_vecOutputMin = 0x1e8; // GlobalTypes::Vector  | Schema_Atomic | Size: 0xc
				constexpr std::ptrdiff_t m_vecOutputMax = 0x1f4; // GlobalTypes::Vector  | Schema_Atomic | Size: 0xc
				constexpr std::ptrdiff_t m_bUseParentDensity = 0x200; // bool m_bUseParentDensity; |  0x200 | Schema_Builtin | Size: 0x1
				constexpr std::ptrdiff_t m_nVoxelGridResolution = 0x204; // int32_t m_nVoxelGridResolution; |  0x204 | Schema_Builtin | Size: 0x4
			}
		}
	}
}
